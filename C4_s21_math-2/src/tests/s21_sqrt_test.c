...
static const double sqrt_array[] = {
...
START_TEST(test_sqrt) {
  double test_number = sqrt_array[_i];
  long double otvet_real = sqrt(test_number);
  long double otvet_my = s21_sqrt(test_number);
...
Suite *suite_sqrt(void) {
  ...
  tcase_add_loop_test(tc_core, test_sqrt, 0, 30);
