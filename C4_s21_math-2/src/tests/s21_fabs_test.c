...
static const double fabs_array[] = {
...
START_TEST(test_fabs) {
  double test_number = fabs_array[_i];
  long double otvet_real = fabs(test_number);
  long double otvet_my = s21_fabs(test_number);
...
Suite *suite_fabs(void) {
  ...
  tcase_add_loop_test(tc_core, test_fabs, 0, 30);
